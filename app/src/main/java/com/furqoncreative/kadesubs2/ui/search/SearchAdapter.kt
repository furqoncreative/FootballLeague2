...
class SearchAdapter(
    ...
    class MatchViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItem(items: Event, listener: (Event) -> Unit, context: Context) {
            ...
            if (items.intHomeScore != null) {
                scoreHome.text = items.intHomeScore.toString()
                scoreAway.text = items.intAwayScore.toString()
            } else {
                scoreHome.text = "-"
                scoreAway.text = "-"
            }
            if (items.idHomeTeam != null) {
                getLogoHome(items.idHomeTeam, context)
                getLogoAway(items.idAwayTeam, context)
            }
        ...
        fun getLogoHome(id: String, context: Context) {
            ApiRepository().services.getDetailTeam(id).enqueue(object :
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        Glide.with(context).load(team?.strTeamLogo).into(logoHome)
                    }
        ...
        private fun getLogoAway(id: String, context: Context) {
            ApiRepository().services.getDetailTeam(id).enqueue(object :
                Callback<TeamResponse> {
                override fun onResponse(
                    call: Call<TeamResponse>,
                    ...
                    if (response.isSuccessful) {
                        Log.d("ID", "" + id)
                        val team: Team? = response.body()?.teams?.get(0)
                        Glide.with(context).load(team?.strTeamLogo).into(logoAway)
                    }
